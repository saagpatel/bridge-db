...
- **DB**: `~/.local/share/bridge-db/bridge.db` (WAL mode, `PRAGMA busy_timeout=15000`). Schema at v12 — v6 added `canonical_key` to `pending_handoffs`; v7 added `source_trust` provenance columns to all four instruction-bearing tables; v8 added `shipped_event_dispositions`; v9 added `session_costs`; v10 added context-section integer `version` CAS, `context_section_export_state`, and durable `write_conflicts` receipts; v11 backfills activity `tags` into `content_index`; v12 adds the `session_classification` sidecar for heuristic cost-routing attribution while keeping `session_costs` as pure actuals. Auth state lives in `principals.json` (not the DB); no schema change for Stage-1 auth.
