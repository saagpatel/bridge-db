...
| Migration backups | Destructive schema migration hook | Verified `.bak`, `.sha256`, and `.meta.json` siblings; pre-verification backups remain `legacy-unverified`; retained `.bak-wal` and `.bak-shm` companions remain separate evidence | Review-only; preserve all | One immutable backup per migration label | Missing/bad manifest, digest, SQLite integrity, or schema evidence blocks destructive migration/reuse. Retained companions without a live primary are surfaced as warnings without weakening a verified current anchor. Raced, unreadable, symlinked, or non-regular companion candidates return `state=unverified` with nullable facts and explicit errors instead of terminating health | Online SQLite backup includes committed WAL state; digest + integrity + schema readback verified. Missing historical provenance and unavailable companion facts are never reconstructed retroactively | `health.evidence_lifecycle.migration_backups` inventories every live primary, retained companion, distinct missing primary, and unverified companion candidate; `status` and `dogfood` summarize companion state | Backup/manifest/metadata/companion deletion requires separate approval |
...
RecoveryAnchorV1 unusable. If a globbed companion disappears, cannot be read,
is a symlink, is not a regular file, or its primary cannot be inspected, the
inventory retains the candidate path with `state="unverified"`, uses `null` for
facts it could not prove, and emits a stable error. Unverified candidates do
not contribute to proven orphan or missing-primary counts, and aggregate
`companion_state` is `unverified` rather than optimistically `attached`.
