...
Paste this into a fresh Claude Code session at `~/Projects/bridge-db`:
...
**Context:** The eval set exists at `~/Projects/bridge-db/eval/semantic_quality_set.json` (copy it there from the Claude.ai outputs directory if not present). It has 20 queries drafted. Most have empty `expected_results` arrays that you need to fill in. Two queries (q005 and q015) are intentionally marked `expected_empty: true` — leave those alone.

The methodology is in `~/Projects/bridge-db/docs/IMPLEMENTATION-PLAN-v2.md` Section 7 if you need reference. Four-tier labels: `must` (clearly answers the query), `should` (strongly relevant), `nice_to_have` (tangentially relevant), `irrelevant` (don't include these in expected_results — absence = irrelevant).
...
1. Replace `"bridge_db_snapshot": "TO_FILL_AT_EVAL_TIME"` with the output of `sha256sum ~/Projects/bridge-db/bridge.db` (just the hash string)
2. Leave `inter_rater_agreement` as `null` — it gets filled after blind rating completes

Save the filled JSON to `~/Projects/bridge-db/eval/semantic_quality_set.json` (overwrite in place).

### Job 3: Generate blind rating prompts

For each of these 5 query IDs — **q002, q008, q011, q016, q018** — generate a separate markdown file at `~/Projects/bridge-db/eval/blind_rating_prompts/<query_id>.md`.
...
- [ ] 5 markdown files exist at `~/Projects/bridge-db/eval/blind_rating_prompts/` — one each for q002, q008, q011, q016, q018
...
### Setup
1. Confirm the 5 files exist at `~/Projects/bridge-db/eval/blind_rating_prompts/q002.md`, `q008.md`, `q011.md`, `q016.md`, `q018.md`
...
4. Copy the JSON to `~/Projects/bridge-db/eval/blind_ratings/<query_id>.json`
...
- Present the final JSON via `present_files` so I can save it back to `~/Projects/bridge-db/eval/`
...
- `~/Projects/bridge-db/eval/semantic_quality_set.json` (modify — fill expected_results)
- `~/Projects/bridge-db/eval/blind_rating_prompts/q002.md` (create)
- `~/Projects/bridge-db/eval/blind_rating_prompts/q008.md` (create)
- `~/Projects/bridge-db/eval/blind_rating_prompts/q011.md` (create)
- `~/Projects/bridge-db/eval/blind_rating_prompts/q016.md` (create)
- `~/Projects/bridge-db/eval/blind_rating_prompts/q018.md` (create)

**Files you create (manual, during blind rating workflow):**
- `~/Projects/bridge-db/eval/blind_ratings/q002.json`
- `~/Projects/bridge-db/eval/blind_ratings/q008.json`
- `~/Projects/bridge-db/eval/blind_ratings/q011.json`
- `~/Projects/bridge-db/eval/blind_ratings/q016.json`
- `~/Projects/bridge-db/eval/blind_ratings/q018.json`
- `~/Projects/bridge-db/eval/primary_ratings/q002.json`
- `~/Projects/bridge-db/eval/primary_ratings/q008.json`
- `~/Projects/bridge-db/eval/primary_ratings/q011.json`
- `~/Projects/bridge-db/eval/primary_ratings/q016.json`
- `~/Projects/bridge-db/eval/primary_ratings/q018.json`

**Future chat creates:**
- `~/Projects/bridge-db/eval/semantic_quality_set_final.json`
...
**Methodology reference:** `~/Projects/bridge-db/docs/IMPLEMENTATION-PLAN-v2.md` Section 7 (or the v2 plan file in your Claude.ai outputs, whichever is more accessible).
