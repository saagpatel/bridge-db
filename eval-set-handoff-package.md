...
**Context:** The historical eval set exists at `~/Projects/bridge-db/semantic_quality_set.json`. The original workflow expected an `eval/` copy, but that live eval directory was never created because the semantic/vector path was closed. It has 20 drafted queries and intentionally unfilled `expected_results`; leave it frozen unless the roadmap is explicitly reopened.
...
1. Replace the placeholder `bridge_db_snapshot` value with the output of `sha256sum ~/Projects/bridge-db/bridge.db` (just the hash string)
2. Leave `inter_rater_agreement` as `null` — it gets filled after blind rating completes

Save the filled JSON to `~/Projects/bridge-db/semantic_quality_set.json` (overwrite in place).
...
- `~/Projects/bridge-db/semantic_quality_set.json` (modify — fill expected_results)
