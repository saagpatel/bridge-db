...
from bridge_db.tenancy import (
    ...
    plan_lifecycle,
    probe_process,
    quarantine_legacy_unknown_lease,
    read_active_leases,
    tenancy_inventory,
...
def test_probe_process_prefers_observable_identity_over_signal_permission(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def observable_identity(_pid: int) -> str:
        return "ps-start:current-protected-process"
    ...
    def refuse_signal(_pid: int, _signal: int) -> None:
...
def test_exact_legacy_unknown_owner_quarantine_requires_missing_identity(
    tmp_path: Path,
) -> None:
    root = tmp_path / "tenancy"
    tracker = TenancyTracker(
        ...
        pid=os.getpid(),
        process_identity="fixture-process-start",
    )
    tracker.start()

    default_plan = plan_lifecycle(
        root=root,
        policy=_policy(),
        current_generation="generation-new",
        process_probe=lambda _pid, _identity: "missing",
    )
    assert default_plan["decisions"][0]["decision"] == "keep_policy_missing"

    path = root / "active" / f"{tracker.lease_id}.json"
    source_sha256 = hashlib.sha256(path.read_bytes()).hexdigest()
    receipt = quarantine_legacy_unknown_lease(
        root=root,
        lease_id=tracker.lease_id,
        source_lease_sha256=source_sha256,
        process_probe=lambda _pid, _identity: "missing",
    )
    assert receipt["effects"][0]["readback"] == "active_absent_history_verified"
    replay = quarantine_legacy_unknown_lease(
        root=root,
        lease_id=tracker.lease_id,
        source_lease_sha256=source_sha256,
        process_probe=lambda _pid, _identity: "unknown",
    )
    assert replay["effects"][0]["readback"] == "already_complete_history_verified"


@pytest.mark.parametrize("state", ["same", "unknown"])
def test_exact_legacy_unknown_owner_stays_fail_closed_when_ambiguous(
    tmp_path: Path,
    state: str,
) -> None:
    root = tmp_path / "tenancy"
    ...
    with pytest.raises(TenancyContractError) as refused:
        quarantine_legacy_unknown_lease(
            ...
            lease_id=tracker.lease_id,
            source_lease_sha256=source_sha256,
            process_probe=lambda _pid, _identity: state,  # type: ignore[return-value]
        )
    assert refused.value.reason_code == "tenancy.process_state_changed"
    assert path.exists()
